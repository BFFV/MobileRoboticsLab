...
class DeadReckoningNav:
    ...
    def obstacle_cb(self, obstacle):
        self.obstacle = obstacle.data

    def apply_velocity(self, lin_vel_list, ang_vel_list, time_list):
        """Applies velocities sequentially. Note that arguments are split
    ...
    def apply_velocity_tuples(self, *vel_tuples):
        """Interface for apply_velocities so we can pass just tuples of
            velocity components"""
        self.apply_velocity(*zip(*vel_tuples))

    def move_robot_to_destiny(self, goal_pose_list):
        """Moves robot to all poses in the list sequentially"""
        # TODO: Refactor
        ...
        while next_pose:
            ...
            # We rotate until we are pointing towards the next pose
    ...
    def wait_and_publish_position(self):
        """For debugging purposes. Waits for an odometry and real pose at the same
